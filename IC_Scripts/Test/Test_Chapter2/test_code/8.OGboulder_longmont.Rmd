...
# Add as single 'coords' column in "(X, Y)" format
usable_sf$coords <- paste0("(", coords_mat[,1], ", ", coords_mat[,2], ")")
...
profile <- LambdaGridSearch(coord_model, 
                            risk_model)
...
# Extract coords and filter
coords_mat <- st_coordinates(usable_sf)[, c("X", "Y")]

df <- usable_sf %>%
  mutate(lon = coords_mat[, "X"],
         lat = coords_mat[, "Y"]) %>%
  filter(
    lon >= lon_min,
    lon <= lon_max,
