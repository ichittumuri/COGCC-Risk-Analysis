...
# =============================================================================
# 4) Downsample 0-risk points to ~5% spills (only from complete rows)
# =============================================================================
set.seed(123)

spills     <- df %>% filter(risk == 1)   # keep all spills as-is
non_spills <- df %>% filter(risk == 0)

# Columns you’ll use in modeling
fac_vars <- c("status","flowline_action","location_type","fluid","material")
num_vars <- c("diameter_in","length_ft","line_age_yr")
cols_used <- c(fac_vars, num_vars)

# Keep only 0-risk rows with no NAs in used columns (and finite numerics)
non_spills_clean <- non_spills %>%
  filter(if_all(all_of(cols_used), ~ !is.na(.))) %>%
  filter(if_all(all_of(num_vars), ~ is.finite(.)))

# Target ~5% spills overall
n_spills <- nrow(spills)
target_total <- ceiling(n_spills / 0.05)
n_non_spills_needed <- max(0, target_total - n_spills)

non_spills_sample <- non_spills_clean %>%
  sample_n(size = min(n_non_spills_needed, nrow(non_spills_clean)))

df_balanced <- bind_rows(spills, non_spills_sample)

cat("\n--- NEW RATIO (~5% spills, 0-risk sampled from NA-free rows) ---\n")
df_balanced %>%
  count(risk) %>%
  mutate(prop = n / sum(n)) %>%
  print()
...
# +   "location_typeManifold",
# +   "location_typeWell Site",
# +   "fluidMultiphase",
# +   "materialFiberglass",
# +   "materialSteel",
# +   "length_ft"
