...
# 11. Post‐process predictions:
#     - apply 0.5 cutoff
#     - override original 1’s to stay 1
df_model <- df_model %>%
  mutate(
    pred_class = as.integer(pred_risk > 0.5),
    pred_final = ifelse(risk == 1, 1L, pred_class)
  )

# 12. Quantify original vs. asserted predictions (using pred_final)
orig_counts       <- table(Original          = df_model$risk)
pred_counts_final <- table(AssertedThreshold = df_model$pred_final)
conf_mat_final    <- table(Observed          = df_model$risk,
                           Predicted         = df_model$pred_final)

print(orig_counts)
print(pred_counts_final)
...
# 14. Map the **final** predictions (with asserted 1’s)
leaflet(df_model) %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  addPolylines(
    color   = ~ifelse(pred_final == 1, "red", "blue"),
    weight  = 2, opacity = 0.7
  ) %>%
  addLegend(
    "bottomright",
    colors = c("blue", "red"),
    labels = c("No Spill", "Spill"),
    title  = "Predicted Risk (assert 1’s)"
  )

# 15. Match vs. mismatch map (using original 0.5 cutoff)
df_model <- df_model %>%
...
# 16. Uncertainty quantification (after confusion matrices)
...
# numeric summaries of uncertainty
